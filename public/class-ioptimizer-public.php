<?php

/**
 * The public-facing functionality of the plugin.
 *
 * @link       https://i-optimizer.com/
 * @since      1.0.0
 *
 * @package    Ioptimizer
 * @subpackage Ioptimizer/public
 */

/**
 * The public-facing functionality of the plugin.
 *
 * Defines the plugin name, version, and two examples hooks for how to
 * enqueue the public-facing stylesheet and JavaScript.
 *
 * @package    Ioptimizer
 * @subpackage Ioptimizer/public
 * @author     Angel Daniel Mainerici <mainerici.angel@gmail.com>
 */
class Ioptimizer_Public {

	/**
	 * The ID of this plugin.
	 *
	 * @since    1.0.0
	 * @access   private
	 * @var      string $plugin_name The ID of this plugin.
	 */
	private $plugin_name;

	/**
	 * The version of this plugin.
	 *
	 * @since    1.0.0
	 * @access   private
	 * @var      string $version The current version of this plugin.
	 */
	private $version;

	/**
	 * Initialize the class and set its properties.
	 *
	 * @param string $plugin_name The name of the plugin.
	 * @param string $version The version of this plugin.
	 *
	 * @since    1.0.0
	 */
	public function __construct( $plugin_name, $version ) {

		$this->plugin_name = $plugin_name;
		$this->version     = $version;

	}

	public function alter_img( $attr ) {
		$attr['loading'] = 'lazy';

		return $attr;
	}

	public function alter_avatar( $avatar ) {
		$avatar = str_replace( '<img ', '<img loading="lazy" ', $avatar );

		return $avatar;
	}

	public function alter_plugin_content( $content ) {
		if ( ! strpos( $content, '<img' ) ) {
			return $content;
		}
		if ( ! preg_match_all( '/<img\s[^>]+>/', $content, $matches ) ) {
			return $content;
		}
		$toReplace   = [];
		$replaceWith = [];
		foreach ( $matches[0] as $imageHtml ) {
			if ( ! strpos( $imageHtml, 'loading=' ) ) {
				$toReplace[]   = $imageHtml;
				$replaceWith[] = str_replace( '<img ', '<img loading="lazy" ', $imageHtml );
			}
		}
		$content = str_replace( $toReplace, $replaceWith, $content );

		return $content;
	}
}
